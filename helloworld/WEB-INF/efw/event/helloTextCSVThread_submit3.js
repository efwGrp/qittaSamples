...
helloTextCSVThread_submit3.mylocker = new java.util.concurrent.locks.ReentrantLock();//マルチスレッドバッファーを操作するためのロッカー
	...
	//例６、マルチスレッドの例
		...
		function makeCsvBuffer(){
			var csvreader=new CSVReader(
			    ...
			    //もしID別の配列がまだ存在しない場合、その配列を初期化する
				helloTextCSVThread_submit3.mylocker.lock();//ロックする
			    	if (buffer[fields[0]]==null)buffer[fields[0]]=[];
			    	buffer[fields[0]].push(fields);
				helloTextCSVThread_submit3.mylocker.unlock();//ロック解除する
			});
		}
		//------以下はバッファー保存用の内部関数
		function saveBuffer(){
